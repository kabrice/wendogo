...
const SEAutoSuggestListInput = (props) => {
  ...
  const [currentPriorityInput, setCurrentPriorityInput] = useState(mainValues.length===0 ? 0 : mainValues.length)
  const [deletedPriority, setDeletedPriority] = useState(null)
  ...
  const handleOnClickOption = (index, e) => { // changeMainObject is a boolean : true if we want to change the main object, false if we want to add a new one
    ...
    if (!mainValues.some(subject => subject.id === suggestions[index].id)) {
      ...
      handleChange(newMainValues) 
      //setMainValues(newMainValues)
      setCurrentPriorityInput(mainValues.length+1)
      setBusy(false);
      reinitializeInput(true)
      setStopSearch(mainValues.length === 2)
    }
  }

  const activateSearchFromTag = (priority, e) => {
    // if (e.defaultPrevented) return
    //   e.preventDefault()
    console.log('priority', priority) 
    setCurrentPriorityInput(priority-1)
    setDeletedPriority(priority)
    console.log('curr', currentPriorityInput) 
    const updatedMainValues = mainValues.filter(item => item.priority !== priority);
    // Do something with updatedMainValues
  ...
  return (
    <div className={`fade-animation fade-slow-enter-done FieldWrapper TextField ${focused ? 'focused' : ''} `} >
      <div className={`FieldView DaisyFieldView TextField COORD_PRE ${focused ? 'focused' : ''}`}>
          ...
          <div className="app-col-xs-12 app-col-sm-8 app-col-md-8 Field ignore-outside-click" >
            <div className="AutoSuggest TextFieldContainer isBordered">
              ...
              {!stopSearch && suggestions?.length>0 && <div {...bindOptions} className="slide-animation slide-fast-enter-done">
                <div className={`AutoSuggest-elements List-elements not-empty`} onClick={e => {console.log('A3')}}>
                  <div className="ListSuggestion"  ref={newRef} onClick={e => {console.log('A2')
                                    // e.preventDefault()
                                    // e.stopPropagation();
                                    // e.nativeEvent.stopImmediatePropagation();
                                   }} >
        ...
        {mainValues.map((item, index) => (
                <div className="AutoSuggest suggestion-tag TextFieldContainer isBordered" key={index}>
                  <div className="AutoSuggest-inputWrapper  ">
                    <div className="Input  hasSymbol right  field-valid ">
                    <i>{`${item.priority}.`}</i> &nbsp; {item.name}
                      <span className="Input-symbol" onClick={(e) => activateSearchFromTag(item.priority, e)}>
        ...
        {mainValues?.length>0 && <div className="Tip subTip " style={{marginTop: 20}}>
...
export default SEAutoSuggestListInput;

...
function SEDropDownList(props) {
    const { title, newRef, collapseOption, items, itemSelected, toggleDropdown, updateSelected, handleContinue, showContinueBtn, tip, showTip, isPartOfInputGroup, inputGroupBlockTitle, svgConstantName, forModal } = props;
    
    return (
        <div className="FieldWrapper SelectField VEH_USA_KILOM field-default" style={forModal && {margin: 0}}>
            <div className="FieldView DaisyFieldView  field-default SelectField VEH_USA_KILOM" style={(isPartOfInputGroup || forModal) && {margin: 0}}>
                {!isPartOfInputGroup
                            ...
                            {!collapseOption && (
                                <>
                                    ...
                                    <div className="slide-animation slide-fast-enter-done">
                                        <div className="Select-elements List-elements not-empty up" style={forModal ? {maxHeight: '120px'} : {}}>
                                            {items.map((item) => (

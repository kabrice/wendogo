...
const ReportCard1 = () => {
    ...
    useEffect(() => {
        const initializeData = () => {
            ...
            const periods = user?.academicYearHeadDetails1.academicYearOrganization?.name === 'Trimestre' ? 4 : 3;
            ...
            const bacYear = parseInt(mostRecentBacId.slice(-1));
            setIsBaccalaureatMarkMandatory(bacYear > 5);
            ...
            setTitleMarkInput(`Veuillez renseigner une nouvelle matière, le ${
                bacYear > 5 ? 'coefficient' : user?.academicYearHeadDetails1?.subjectWeightSystem?.name?.toLowerCase()
    ...
    return (
        <>
            ...
            {isReadModes.every(mode => mode) && showContinueBtn && 
                ...
                <div className="FieldView field-valid">
                            <ButtonLarge name="Continuer" handleContinue={handleContinue} uniqueId={`ReportCard1-continue-btn`}/></div></>}
            {renderSEMarkInputs()}

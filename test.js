...
const SimulationEngine = ({ spokenLanguages, academicYearOrganizations, markSystems, schoolYears, subjectWeightSystems, universityLevels, degrees, isErrorPage }) => {
    ...
    useEffect(() => {
        const initializeData = async () => {
            try {
                ...
                // Set up all the necessary state
                setUser(userData);
                dispatch(setStep(userData?.simulationStep ?? SIMULATION_ENGINE_STEPS.VISA_TYPE));
                dispatch(setProgress(userData?.progressBarStep ?? PROGRESS_BAR_STEPS.GENERALITES_SUR_LES_VISAS_ET_LES_ETUDES));
                dispatch(userData?.schoolLevelSelected === 'Supérieur' ? activateUniversity() : deactivateUniversity());
                dispatch(userData?.hsLevelSelected === 'deg00002' ? activatePremiereClass() : deactivatePremiereClass());
            } catch (error) {
    ...
    const handleProgressBarStep = (stepId) => { 
        setUser((prevUser) => {
            console.log('PPPPP', prevUser.selectedFrenchLevel)
            const updatedUser = {
                ...prevUser,
            ...
            return updatedUser; // Update the state with the new user
        });
